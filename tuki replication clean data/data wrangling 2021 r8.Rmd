...
# load all wave 8 variables

```{r}
data = read_sav("/Users/andyouyang/Documents/research/Mali/data/afrobarometer_mli_r8_2021.sav")
```
...
violence <- subset(violence, violence$year < 2021)
...
```{r}
new_data$vio_97_21[new_data$REGION == 509] <- 60 # bamako
new_data$vio_97_21[new_data$REGION == 508] <- 445 # kidal
new_data$vio_97_21[new_data$REGION == 507] <- 612 + 233 # Gao + Menaka
new_data$vio_97_21[new_data$REGION == 506] <- 429 # tombouctou
new_data$vio_97_21[new_data$REGION == 505] <- 1357 # mopti
new_data$vio_97_21[new_data$REGION == 504] <- 226 # segou
new_data$vio_97_21[new_data$REGION == 503] <- 39 # sikasso 
new_data$vio_97_21[new_data$REGION == 502] <- 49 # koulikoro
new_data$vio_97_21[new_data$REGION == 501] <- 32 # kayes
```
...
```{r}
write.csv(new_data,"~/Documents/research/clean_mali_data_round8.csv", row.names = FALSE)
```
