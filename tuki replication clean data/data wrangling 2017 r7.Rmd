...
# load all wave 7 variables

```{r}
data = read_sav("/Users/andyouyang/Documents/research/Mali/data/afrobarometer_mli_r7_2017.sav")
```
...
violence <- subset(violence, violence$year < 2017)
...
```{r}
new_data$vio_97_21[new_data$REGION == 509] <- 41 # bamako
new_data$vio_97_21[new_data$REGION == 508] <- 273 # kidal
new_data$vio_97_21[new_data$REGION == 507] <- 192 + 41 # Gao + Menaka
new_data$vio_97_21[new_data$REGION == 506] <- 185 # tombouctou
new_data$vio_97_21[new_data$REGION == 505] <- 109 # mopti
new_data$vio_97_21[new_data$REGION == 504] <- 32 # segou
new_data$vio_97_21[new_data$REGION == 503] <- 9 # sikasso 
new_data$vio_97_21[new_data$REGION == 502] <- 16 # koulikoro
new_data$vio_97_21[new_data$REGION == 501] <- 16 # kayes
```
...
```{r}
new_data$night_2020mean[new_data$REGION == 509] <- 46.99025933327528 # bamako
new_data$night_2020mean[new_data$REGION == 508] <- 0.0025426971980411386 # kidal
new_data$night_2020mean[new_data$REGION == 507] <- 0.007114445656458759 + 0.0002457712387672023
# gao and menaka
new_data$night_2020mean[new_data$REGION == 506] <- 0.0025434696654836842 # tombouctou
new_data$night_2020mean[new_data$REGION == 505] <- 0.024054927986292836 # mopti
new_data$night_2020mean[new_data$REGION == 504] <- 0.061153232962202514 # segou
new_data$night_2020mean[new_data$REGION == 503] <- 0.049531837968388115 # sikasso
new_data$night_2020mean[new_data$REGION == 502] <- 0.13272089071165044 # koulikoro
new_data$night_2020mean[new_data$REGION == 501] <- 0.04954024596493297 # kayes
...
```{r}
write.csv(new_data,"~/Documents/research/clean_mali_data_round7.csv", row.names = FALSE)
```
